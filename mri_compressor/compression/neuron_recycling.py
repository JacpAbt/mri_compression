...
class DeadNeuronMap:
    ...
    def total_trainable_params(self) -> int:
        ...
        for lm in self.layer_maps.values():
            n = lm.n_recyclable
            # gate_proj: n rows x hidden_size
            # up_proj:   n rows x hidden_size
            # down_proj: hidden_size x n cols
            # Total: n x hidden_size x 3
...
def build_dead_neuron_map_from_activations(
    ...
    - dead: mean |activation| < dead_threshold x layer_mean_magnitude
...
def cmd_prepare(args):
    ...
    from .compressor import collect_activations  # Use local import for calibration utilities
    from torch.utils.data import DataLoader

    logger.info("Computing dead neuron map from activations...")
    # Build a simple calibration dataloader
    from datasets import load_dataset
    ds = load_dataset("Salesforce/wikitext", "wikitext-103-raw-v1", split="test")
    texts = [s["text"] for s in ds if len(str(s["text"]).strip()) > 50][:64]

    class SimpleDS:
        def __init__(self, texts, tok, max_len=512):
            self.data = []
            for t in texts:
                enc = tok(t, return_tensors="pt", truncation=True, max_length=max_len, padding="max_length")
                self.data.append({"input_ids": enc["input_ids"][0], "attention_mask": enc["attention_mask"][0]})
        def __len__(self): return len(self.data)
        def __getitem__(self, i): return self.data[i]

    cal_ds = SimpleDS(texts, tokenizer)
...
def cmd_finetune(args):
    ...
    _ds = _load_dataset("Salesforce/wikitext", "wikitext-103-raw-v1", split="test")
    _texts = [s["text"] for s in _ds if len(str(s["text"]).strip()) > 50][:64]
    ...
    cal_loader = DataLoader(_SimpleDS(_texts, tokenizer), batch_size=4, shuffle=False)
...
def cmd_analyze_routing(args):
    ...
    cal_loader = DataLoader(SimpleDS(texts, tokenizer), batch_size=4, shuffle=False)
